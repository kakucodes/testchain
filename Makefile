...
HTTPS_GIT := github.com/testchain/rollchain.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=rollchain \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=kakud \
...
ifeq ($(OS),Windows_NT)
	...
	go build -mod=readonly $(BUILD_FLAGS) -o build/kakud ./cmd/kakud
endif

build-windows-client: go.sum
	GOOS=windows GOARCH=amd64 go build -mod=readonly $(BUILD_FLAGS) -o build/kakud.exe ./cmd/kakud
...
install: go.sum
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/kakud
...
draw-deps:
	...
	@goviz -i ./cmd/kakud -d 2 | dot -Tpng -o dependency-graph.png
...
set-testnet-configs:
	kakud config set client chain-id chainid-1
	kakud config set client keyring-backend test
	kakud config set client output text

# import keys from testnet.json into test keyring
setup-testnet-keys:
	-`echo "decorate bright ozone fork gallery riot bus exhaust worth way bone indoor calm squirrel merry zero scheme cotton until shop any excess stage laundry" | kakud keys add acc0 --recover`
	-`echo "wealth flavor believe regret funny network recall kiss grape useless pepper cram hint member few certain unveil rather brick bargain curious require crowd raise" | kakud keys add acc1 --recover`
