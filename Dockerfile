...
# then log output of file /code/bin/kakud
# then ensure static linking
RUN LEDGER_ENABLED=false BUILD_TAGS=muslc LINK_STATICALLY=true make build \
  && file /code/build/kakud \
  && echo "Ensuring binary is statically linked ..." \
  && (file /code/build/kakud | grep "statically linked")

# --------------------------------------------------------
FROM alpine:3.16

COPY --from=go-builder /code/build/kakud /usr/bin/kakud
